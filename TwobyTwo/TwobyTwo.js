...
//was added by Andy Dean, andy.dean (at) gmail.com, who also added a JavaScript
//version of David Martin's exact calculations.
var Title="TwobyTwo Tables";

var Authors="<b>Statistics</b><br>Kevin M. Sullivan, Emory University<br>";
    Authors+= "and Andrew G. Dean, EpiInformatics.com<br>";
    Authors+= "based on code from John C. Pezzullo<br>";
	Authors+= "Exact and maximum likelihood statistics ";
	Authors+= "adapted from a Pascal program by David Martin.";
    Authors+= "Thanks to Ray Simons for advice and testing.<br>";
    Authors+="<b>Interface</b><br>";
    Authors+="Andrew G. Dean and Roger Mir<br>";
var Description="<p>Two by two tables are used to evaluate the association between a possible "
    Description+="risk factor (&quot;Exposure&quot;) and an outcome (&quot;Disease&quot;). "
    Description+="Counts summarizing the occurence of the four possible combinations of "
    Description+="events in the study population are entered into the appropriate cells. "
    Description+="The table can be rotated or flipped  so that either rows or "
    Description+="columns represent Exposure, and the column headings (+) and (-) can be "
    Description+=" in either order "
    Description+="to match common textbooks of epidemiology. A single table or multiple "
    Description+="strata can be entered. </p> "
    Description+="<p>Statistics produced include the Fisher and mid-p exact tests, chi squares, "
    Description+="odds ratio, maximum likelihood odds ratio estimate, risk/prevalence ratio "
    Description+="(relative risk), risk difference, and etiologic fractions with confidence "
    Description+="limits produced by several methods, with stratified analysis "
   
//The text in the next variables will be inserted into the HTML document that comes up in response to the Exercise link
var Demo="A cohort study of 162 men and 347 women examines the relationship between low income (the exposure) and a disease"+
...
var Exercises="<h2>Exercise 1--A Case-Control Study</h2>"+
...
function doStatistics(cmdObj,showTables, showStrata, casecontrol)
{
/*
...
if (numStrata>1)
{
   ...
   // alert(limits(lowRRDirect,upRRDirect,1))
	...
	// riskBased[index]+='\nnewrow("span6: Breslow-Day test for interaction of Risk Difference over strata:: chi square='+fmtSigFig(BDchiRD,4)+ '"," p='+pBDchiRD+'")'
     ...
     if ((pBDchiRD<0.05)&&(!( pBDchiRD.indexOf(">")) ))
	 {
     //Jan 2013 Added test to exclude initial ">"
...
if ((cmdObj.data[0].strata==1) && ((a == 0) || (b == 0) || (c == 0) || (d == 0)))
{ zeromsg= "Some calculations are not possible with a zero cell. After seeing these results you may wish to add a small value like 0.5 to each cell and calculate again."
alert(zeromsg)
}


//Chi Square calculations         
  cs=(t * Math.pow((a * d) - (b * c),2))/(c1 * c2 * r1 * r2);
...
} //end doStatistics function : Note nesting of functions
