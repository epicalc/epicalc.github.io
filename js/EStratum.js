function Stratum(x, y, w, h, r, c, name) {
  ...
  // this.addCalendar=StratumAddCalendar;
  ...
  this.moveInputHere = StratumMoveInputHere
...
function addCalendar() {
  jQuery(function()
    {
      /* JSG: Functions are "first class objects" in JS,
    thus, we can store "functional level" variables inside of them,
    to reduce with global namespce clutter.

    Here, we assign a "currentCalendar" instance and refer to elsewhere too, in
    removeCalendar.
   */
      addCalendar.currentCalendar = jQuery("#entryfield").datepicker(
        {
        yearRange: '-99:+00',
        changeMonth: true,
        changeYear: true
      ...
      addCalendar.currentCalendar.bind('change', function() {addCalendar.currentCalendar.focus()});
      addCalendar.currentCalendar.focus();
    }
  );
...
{
   if (typeof (jQuery("#entryfield").datepicker)!='undefined')
   {
...
function StratumMoveInputTo(r, c) {
  ...
  if ((tp !="data") && (tp !="calendar"))
  {
    ...
    //except for calendar and data fields
    else if (tp=="valname") {tp = valnamemsg}
    else if (tp=="varname") {tp = varnamemsg}
    else if (tp=="label") {tp = labelmsg}
    {
      enter = window.confirm(entermsg+" " + tp + "?")
    }
  }
  else
  {
    enter = true //Do not require permission
  }

  if (enter)
  {
    //If cell is a data cell, then make the input box visible
    ...
    //Move box into place over correct part of the table
    this.inputbox.moveTo(this.row[r].cell[c].dynlayer.x, this.row[r].cell[c].dynlayer.y);


    var entryObj = jQuery("#entryfield");
   /* if (tp=="calendar")
    {
      addCalendar();
    }
    ...
    */
    entryObj.focus();
    //Have EntryObj assume the value currently in the table
    entryObj.val(this.row[r].cell[c].value)
    entryObj.select()
...
function StratumMoveInputHere()
{
  //New function, May 2012
  moveInputTo(this.inputR, this.inputC);
}
...
function StratumMoveInputNext()
{
  ...
  col = this.inputC
  tries = 0;
  // The following loop searches for the "next" data or calendar cell
  //****************************************************************
  do
  {
    tries += 1;
  ...
  //******************************End of search*******************
  //and puts the cursor in the data or calendar field that is found, or at row 0, column 0 if none is found.
...
function StratumEvalEntry()
{
  ...
  //var inputForm=document.inBox;      //previous version

 // Commented out as EXPERIMENT
 var inputForm = $("inBox");
 //var inputForm= jQuery('#inBox') //Another EXPERIMENT
try
 {
...
function StratumBuild() {
 ...
 // if (currConfig() != "smartphone")
 if (typeof customInputDiv == 'function')
   {
    this.div += customInputDiv();
   }
   else
   {
...
*/
  ...
  this.div += '</div>'
  this.div += '</div>'  //end of dataTableDiv
...
function StratumOut(r, c)
{
  ...
  //Apparently when entry box gets in front of a cell, it causes mouseout and the cursor focus and selection have to be restored.
}
