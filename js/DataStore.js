...
for(r=0; r<myTable.rows; r++) 
 {
   ...
   for (c=0; c<myTable.cols; c++)
    {    
     	 ...
     	 v1=myTable.get(r,c);  //get cell value
...
function strIdFromRC(r,c)
{
var d
var e
if (!useTableSettings)
{
 r=r-dataMatrix[0].datarmin
 c=c-dataMatrix[0].datacmin
 return "E"+r+"D"+c 
}
if (MaxValueAtTop)
  {
...
if (ExposureLeft)
  {
  e=r-dataMatrix[0].datarmin //Set r=E number
  d=c-dataMatrix[0].datacmin //Set c=D number
  }
else
  {
  e=c-dataMatrix[0].datacmin  //Set c=E number , i.e., swap rows and columns
  d=r-dataMatrix[0].datarmin   //Set r=D number
  } 
  return "E"+e+"D"+d 
}

function rcFromStrId(strId)
{
//Returns an array containing r and c in that order
//Note that the results are not confined to the data cells, but include the 
//entire table.
var r=0;
var c=0;
var epos
var dpos
dpos=strId.search(/D/i)
epos=strId.search(/E/i)
if (dpos < 0 ||epos < 0)
  {return ""}
r=parseInt(strId.substring(1,dpos-epos))  //Set r=E number
c=parseInt(strId.substring(dpos+1))     //Set c=D number
if (useTableSettings)
{
if (ExposureLeft)
  {
  ...
  {
  c=parseInt(strId.substring(1,dpos-epos))  //Set c=E number , i.e., swap rows and columns
  r=parseInt(strId.substring(dpos+1))     //Set r=D number
  } 
if (MaxValueAtTop)
  {
   //Reverse row order
