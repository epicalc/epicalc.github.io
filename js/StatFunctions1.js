// JavaScript Document    StatFunctions1.js

//Associative array sort added at end, January 2014
...
function coloredText(text,color)
{
 ...
 //return "<span style='color:"+color+"'>"+text+"<\/span>";  //Not deprecated, but doessn't work
  ...
  {     //Checks for valid date.  Requires that data string has at least one slash
    if (sval.indexOf("/")>0)
    {
      var d = new Date(sval);
 ...
 function Timeline (numArray)
 {
  ...
  //sort numerically
  var TL=Array();
  TL.meta="min,max,interval,label,unit,data"
  TL.min=numArray[0] //Minimum
  TL.max=numArray[numArray.length-1] // Maximum
  TL.interval=TL.max-TL.min;

   //Convert to seconds, minutes, hours, days, weeks, months, years, or decades
   var second=1000;
   var minute=60000
   var hour=60*minute;
   var day=24*hour;
   ...
   var label="";
   var unit=1;
   if (TL.interval/second < 100)
     { label="Seconds";unit=second;}
     else if (TL.interval/minute < 100)
     { label="Minutes";unit=minute;}
     else if (TL.interval/hour < 100)
     { label="Hours";unit=hour;}
     else if (TL.interval/day < 100)
     { label="Days";unit=day;}
     else if (TL.interval/week < 100)
     { label="Weeks";unit=week;}
     else if (TL.interval/month < 100)
     { label="Months";unit=month;}
     else if (TL.interval/year < 100)
     { label="Years";unit=year;}
    else if (TL.interval/decade < 100)
     { label="Decades";unit=decade;};
   TL.label=t(label);  //Text suitable for horizontal axis label
   TL.unit=unit;  //The chosen unit in milliseconds
   TL.data=numArray;
   TL.intervalunits=Math.floor(TL.interval/TL.unit)
  return TL;
    //Note: TL has properties named in the TL.meta string
 }

 function BW5Params(numArray)
  {
  //Provides Min,25pctile,50pctile,75pctile,Max for Tukey style Box Whisker.  Percentile
  //calculations and the BW itself seems to have several versions.
  //Returns an array, with the 0 index being a text string that describes the items
  ...
  //sort numerically
  var BW=Array();
  BW[0]="Min,25pctile,50pctile,75pctile,Max"
  BW.min=numArray[0] //Minimum
  BW.p25=calcPctile(25); //25th percentile
  BW.median=calcPctile(50); //50th percentile (median)
  BW.p75=calcPctile(75); //75th percentile (median)
  BW.max=numArray[numArray.length-1] // Maximum
  return BW;

    function calcPctile (pct)
    {
    //must be inside percentile function as it uses the sorted numArray in that function
    var result=-1;
   // var Rank=(pctile/100) * (n+1);
    var Rank=((n*pct)/100) +0.5    //From Ian Robertson, "Calculating Percentiles"
    var IR=parseInt(Rank) //The Integer portion of IR
    if (IR==n) { return numArray[n-1]}  //Avoids error -- believe algorithm was flawed
    if (IR==0)
      {
       IR=1;
      }
    var FR=Rank-IR   //The fractional portion of IR

    if (FR===0)
    {
      //Rank is an integer
      result = numArray[Rank-1]
    }
    else
    {
      result = Number(numArray[IR-1])+ Number(((numArray[IR]-numArray[IR-1])*FR));
      //note adjustment for 0-based array
    }
    //alert("result=" + result)
    return result
    } // end calcPctile
...
function proportion95CL(vx,vN,type)
 ...
 var anObject={"B":2022,"C":33,"A":11,D:44}

function testArraySortByIdx()
{
 showObject(anObject)
 showObject(sortedByIdx(anObject))

}

function getObjectLength (o) {
  //Returns the number of items in an associative array,
  //not counting the normal mehods and properties of the object
  var size = 0;
  ...
  return size-2;          //Experiment, as it seems to return too many!!
}


function sortedByIdx(arrToSort,chrToSkip)
{
 //Given an associative array, produces a copy sorted by array index IGNORING the initial number
// of characters specified.
 //Hence V1  and V10  will sort as though they were 1 and 10
 if (chrToSkip==null )
   {
    chrToSkip=0;
   }
 ...
 for (var idx in arrToSort)
       {
        // if (Object.prototype.hasOwnProperty.call(arrToSort, idx))
         if(arrToSort.propertyIsEnumerable(idx))
         {
           idxArr.push(idx)
         }
       }
       //Now we have a list of the user-defined indices (not those inherited from object prototype)

  // idxArr=idxArr.sort();  //Sort the list
 // idxArr.sort(function(a,b){return a.substr(chrToSkip)-b.substr(chrToSkip);});
 //numeric sort skipping the specified number of characters.
    idxArr.sort(function(a,b){return (b > a)});

  for ( var i=0;i<idxArr.length;i++)
      {
      sortedArr[idxArr[i]]=arrToSort[idxArr[i]];
      }
...
{
//Shows associative array in alert box.
var result="";
for (var name1 in theObject)
 {
 result+=name1 +": "+theObject[name1]+"\n"
 }
 alert (">>\n"+result);
 return result;
}